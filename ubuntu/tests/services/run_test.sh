...
echo -e "${YELLOW}[STEP 1] Unsecuring all services settings...${RESET}"
bash "$UNSECURE_SCRIPT"

# Load environment dependencies
load_dpkg
load_severity "strict"
load_json_services "$SCRIPT_DIR/../../config/services.json"
...
for script in "${AUDIT_SCRIPTS[@]}"; do
    ...
    printf "  Audit: %-52s | Expected: %-12s | Actual: ${COLOR}%-14s${RESET} | Status: ${COLOR}%s${RESET}\n" \
...
echo -e "\n${CYAN}=========================================================================================================${RESET}"
echo -e "${CYAN}                                       SERVICES MODULE TEST RESULTS                                      ${RESET}"
echo -e "${CYAN}=========================================================================================================${RESET}"
...
for script_id in "${!SCRIPT_NAMES[@]}"; do
    name="${SCRIPT_NAMES[$script_id]}"
    p1_act="${INITIAL_ACTUAL[$script_id]:-UNKNOWN}"
    p2_act="${FINAL_ACTUAL[$script_id]:-UNKNOWN}"
