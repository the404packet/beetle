#!/usr/bin/env bash
# =============================================================================
# harden/host_based_firewall/4.2/ensure_ufw_open_ports_have_rules.sh
# CIS Ubuntu Benchmark — 4.2.6
# Ensure ufw firewall rules exist for all open ports (Automated)
#
# Note: Adds generic ALLOW per port — review rules after hardening
# =============================================================================
...
for port in $open_ports; do
    echo "$ufw_rules" | grep -qw "$port" && continue
    ufw allow "$port" &>/dev/null || failed=1
done
[ "$failed" -eq 0 ] \
