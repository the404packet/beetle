...
# Permissive umask pattern — any umask less restrictive than 027
...
for f in "${root_umask_files[@]}"; do
    [ -f "$f" ] || continue
    # Comment out any permissive umask lines
    if grep -Psi -- "$permissive_pattern" "$f" >/dev/null 2>&1; then
        sed -i -E "s|^(\h*umask\h+.*)$|# \1  # commented out by beetle|gI" "$f"
    fi
done

# Validate — none of the files should have a permissive umask remaining
flag=1
for f in "${root_umask_files[@]}"; do
    [ -f "$f" ] || continue
    if grep -Psi -- "$permissive_pattern" "$f" >/dev/null 2>&1; then
